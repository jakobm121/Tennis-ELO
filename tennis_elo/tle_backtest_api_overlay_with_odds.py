...
def clean_decimal_odds(value: Any, min_odds: float, max_odds: float) -> float | None:
...
def side_values_clean(
    ...
    return [
        number
        for number in (clean_decimal_odds(value, min_odds, max_odds) for value in side_odds.values())
...
def chosen_book_odds(
    ...
    side_odds = market.get(side) or {}
    value = clean_decimal_odds(side_odds.get(bookmaker), min_odds, max_odds)
    ...
    available = side_values_clean(market, side, min_odds, max_odds)

    if not available:
        return None, None

    if fallback in {"clean_average", "avg", "average"}:
        return float(sum(available) / len(available)), "fallback_clean_average"

    if fallback in {"clean_median", "median"}:
        return float(statistics.median(available)), "fallback_clean_median"

    if fallback == "max":
        return max(available), "fallback_max_clean"

    if fallback == "min":
        return min(available), "fallback_min_clean"
...
def side_pair_odds(
    ...
    home_odds, home_book = chosen_book_odds(
        market, "Home", bookmaker, fallback, min_odds, max_odds
    )
    away_odds, away_book = chosen_book_odds(
        market, "Away", bookmaker, fallback, min_odds, max_odds
    )
    ...
    home_values = side_values_clean(market, "Home", min_odds, max_odds)
    away_values = side_values_clean(market, "Away", min_odds, max_odds)
    ...
    if fallback in {"clean_average", "avg", "average"}:
        return (
            float(sum(home_values) / len(home_values)),
            float(sum(away_values) / len(away_values)),
            "fallback_clean_average_pair",
        )
    ...
    if fallback in {"clean_median", "median"}:
        return (
            ...
            "fallback_clean_median_pair",
        )

    if fallback == "max":
        return max(home_values), max(away_values), "fallback_max_pair_clean"

    if fallback == "min":
        return min(home_values), min(away_values), "fallback_min_pair_clean"
...
def main() -> None:
    ...
    parser.add_argument(
        "--fallback",
        choices=["none", "clean_average", "clean_median", "median", "max", "min"],
    ...
    for prediction in predictions:
        ...
        if overround < args.min_overround or overround > args.max_overround:
            counters["skipped_bad_overround"] += 1
            continue
    ...
    print("TLE API OVERLAY ODDS BACKTEST V4 CLEAN DONE")
